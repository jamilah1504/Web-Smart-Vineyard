...
exports.updatePumpStatus = async (req, res) => {
  const { id } = req.params; // Berisi "ESP32-MAC-A001"
  ...
  try {
    ...
    // Pemilah data dinamis agar tidak menimpa kolom lain dengan 'undefined'
    const updateData = {};
    if (mode_kerja !== undefined && mode_kerja !== null) {
        const cleanMode = String(mode_kerja).toLowerCase().trim();
        if (cleanMode === 'auto' || cleanMode === 'otomatis') {
            updateData.mode_kerja = 'auto';
        } else if (cleanMode === 'manual') {
            updateData.mode_kerja = 'manual';
        } else {
            updateData.mode_kerja = cleanMode;
        }
    }
    ...
    try {
        ...
        // Catat histori pompa pupuk jika ada perubahan status
    ...
    return res.status(200).json({ status: 'success', message: 'Kontrol berhasil diperbarui' });
